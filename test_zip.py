...
def test_txt_in_zip():
    text_size = os.path.getsize(os.path.join(RESOURCES_PATH, "file_test_0.txt"))

    with open(os.path.join(RESOURCES_PATH, "file_test_0.txt"), encoding='latin-1') as f:
        txt_file_text = f.read()

    with zipfile.ZipFile(os.path.join(TMP_PATH, 'test.zip')) as txt_z:
        assert text_size == txt_z.getinfo('file_test_0.txt').file_size
        assert txt_file_text in txt_z.read('file_test_0.txt').decode('latin-1')
...
def test_xls_in_zip():
    book = open_workbook(os.path.join(RESOURCES_PATH, "file_test_3.xls"))
    size_of_xls = os.path.getsize(os.path.join(RESOURCES_PATH, "file_test_3.xls"))
    ...
    with zipfile.ZipFile(os.path.join(TMP_PATH, 'test.zip')) as xls_z:
        assert size_of_xls == xls_z.getinfo("file_test_3.xls").file_size
        assert number_of_sheets_xls == open_workbook(file_contents=xls_z.read("file_test_3.xls")).nsheets
        assert names_of_sheets_xls == open_workbook(file_contents=xls_z.read("file_test_3.xls")).sheet_names()
        assert number_of_rows_xls == open_workbook(file_contents=xls_z.read("file_test_3.xls")).sheet_by_index(0).nrows
        assert number_of_rows_cols_xls == open_workbook(file_contents=xls_z.read("file_test_3.xls")).sheet_by_index(0).ncols
        assert sheet_value_xls == open_workbook(file_contents=xls_z.read("file_test_3.xls")).sheet_by_index(0).cell_value(3, 3)


def test_xlsx_file_in_zip():
    workbook = load_workbook(os.path.join(RESOURCES_PATH, "file_test_2.xlsx"))
    size_of_xlsx = os.path.getsize(os.path.join(RESOURCES_PATH, "file_test_2.xlsx"))
    ...
    with zipfile.ZipFile(os.path.join(TMP_PATH, 'test.zip')) as xlsx_z:
        assert size_of_xlsx == xlsx_z.getinfo("file_test_2.xlsx").file_size
        assert number_of_sheets_xlsx == len(load_workbook(xlsx_z.open("file_test_2.xlsx")).sheetnames)
        assert names_of_sheets_xlsx == load_workbook(xlsx_z.open("file_test_2.xlsx")).sheetnames
        assert sheet_value_xlsx == load_workbook(xlsx_z.open("file_test_2.xlsx")).active.cell(row=3, column=3).value
